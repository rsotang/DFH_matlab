...
%%
% Parámetros del volumen y de la esfera del ct0
volSize = [512, 512, 512];   % Tamaño del volumen
outerRadius = 115;  % Radio exterior de la corteza esférica
innerRadius = outerRadius * 0.7; % Radio interior (define el grosor de la corteza)

% Generar el volumen
volumeCT0 = generateSphereCT(volSize, outerRadius, innerRadius);

% Mostrar el volumen 3D
figure;
volshow(volumeCT0);  % Mostrar el volumen en 3D (requerido Matlab R2020b o posterior)

% Mostrar el corte central
showCentralSlice(volumeCT0);  % Llamar a la función para visualizar el corte central

% Parámetros del volumen y de la esfera del ct100
outerRadius = 135;  % Radio exterior de la corteza esférica
innerRadius = outerRadius * 0.9; % Radio interior (define el grosor de la corteza)

volumeCT100 = generateSphereCT(volSize, outerRadius, innerRadius);

% Mostrar el volumen 3D
figure;
volshow(volumeCT100);  % Mostrar el volumen en 3D (requerido Matlab R2020b o posterior)

% Mostrar el corte central
showCentralSlice(volumeCT100);  % Llamar a la función para visualizar el corte central
%%
